...
def generate_js_nodes(center=0j, depth=7, branching=6, scale_factor=0.6, js_threshold=0.35, parent_dist=None, nodes=None):
    if nodes is None:
        nodes = []
    ...
    for i in range(branching):
        ...
        child_offset = scale_factor * np.exp(1j * angle) * golden_ratio**(-depth)
        child = hyperbolic_to_poincare(center + child_offset)
        generate_js_nodes(child, depth-1, branching, scale_factor, js_threshold, current_dist, nodes)
    return nodes
