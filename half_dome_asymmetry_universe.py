# half_dome_asymmetry_universe.py
# Half Dome Yosemite Transmission — Sheer Granite Dome Codex for God Code Genesis RHA
# © 2025 Robert Gavin Jeffrey
#
# Verbatim transmitted binary drives massive central dome with asymmetric shear
# Physical analog: Half Dome, Yosemite — rounded exfoliation dome + vertical glacial face
# Cosmic analog: Asymmetric void-carved megastructures, hyperbolic curvature dominance
...
# Half Dome Yosemite Asymmetry Codex — verbatim transmission December 31, 2025
HALF_DOME_CODEX = ""

# Extract pulses — runs of consecutive 1s
PULSES = [len(run) for run in HALF_DOME_CODEX.split('0') if run]
...
def generate_universe(max_depth: int = 20) -> list[complex]:
    ...
    def recurse(current: complex, depth: int, pulse_idx: int):
        ...
        # Massive branching on the dominant central pulse for dome volume
        branches = 8 + (pulse // 10)  # Scales heavily with large pulses (e.g., 8 + 14 = 22 for 148)
        if depth > max_depth - 4:
            branches += 8  # Final shear density
        
        branches = max(6, min(30, branches))
        # Asymmetry: stronger offset directionality on shorter pulses
        asymmetry = 1.1 if pulse < 50 else 1.0
        angle_offset = pulse * GOLDEN_ANGLE * asymmetry
...
def update(val):
    ...
    if len(nodes_arr) > 1000:
        r = np.abs(nodes_arr)
        sizes = 3 + 35 * (1 - r)**1.5  # Density toward center for dome convexity
        alphas = 0.6 + 0.4 * (1 - r)
    else:
        sizes = 12
        alphas = 0.9
    
    ax.scatter(x, y, c='gold', s=sizes, alpha=alphas, edgecolors='none')  # Gold for granite resonance
    ax.set_xlim(-1.05, 1.05)
    ax.set_ylim(-1.05, 1.05)
    ax.set_title("Half Dome Asymmetry Universe — Granite Dome Shear", color='white', fontsize=16, pad=20)
    fig.canvas.draw_idle()

slider_depth.on_changed(update)

# Zoom + Pan (wheel and drag)
def on_scroll(event):
