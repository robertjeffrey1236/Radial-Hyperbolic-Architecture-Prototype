# bell_rock_vortex_universe.py
# Bell Rock Sedona Transmission — Vortex Spiral Codex for God Code Genesis RHA
# © 2025 Robert Gavin Jeffrey
#
# Verbatim transmitted binary drives layered upward-spiraling vortex resonance
# Physical analog: Bell Rock butte, Sedona — upward energy vortex, layered sandstone bell form
# Cosmic analog: Converging spiral inflows, electromagnetic uplift filaments
...
# Bell Rock Sedona Vortex Codex — verbatim transmission December 31, 2025
...
PULSES = [len(run) for run in BELL_ROCK_CODEX.split('0') if run]
...
def generate_universe(max_depth: int = 20) -> list[complex]:
    ...
    def recurse(current: complex, depth: int, pulse_idx: int):
        ...
        branches = 6 + (pulse % 7)  # Favor higher branching for vortex uplift (6–12 average)
        if depth > max_depth - 6:
            branches += 6  # Stronger late convergence
        
        branches = max(5, min(25, branches))
        angle_offset = pulse * GOLDEN_ANGLE * 1.3  # Enhanced spiral twist for vortex flow
        
        for i in range(branches):
            angle = i * (2 * np.pi / branches) + angle_offset
            offset = 0.62 * np.exp(1j * angle) * (GOLDEN_RATIO ** -depth)
...
def update(val):
    ...
    if len(nodes_arr) > 1000:
        r = np.abs(nodes_arr)
        sizes = 4 + 30 * (r / r.max())**2
        alphas = 0.5 + 0.5 * (1 - r)
    else:
        sizes = 12
        alphas = 0.9
    
    ax.scatter(x, y, c='magenta', s=sizes, alpha=alphas, edgecolors='none')  # Magenta for vortex energy
    ax.set_xlim(-1.05, 1.05)
    ax.set_ylim(-1.05, 1.05)
    ax.set_title("Bell Rock Vortex Universe — Upward Spiral Resonance", color='white', fontsize=16, pad=20)
    fig.canvas.draw_idle()

slider_depth.on_changed(update)

# Zoom + Pan (wheel and drag) — copy from previous scripts
def on_scroll(event):
